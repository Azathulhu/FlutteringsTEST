...
class LevelService {
  ...
  Future<void> completeSubLevel(int subLevelId) async {
    ...
    if (nextSubLevel != null) {
      ...
      // If there is no next sub-level, unlock the default sub-level of the next level
      ...
      if (nextLevel != null) {
        final defaultNextSub = await supabase
            .from('sub_levels')
            .select()
        ...
        if (defaultNextSub != null) {
          await supabase.from('user_levels').upsert({
            'user_id': user.id,
            'sub_level_id': defaultNextSub['id'],
            'is_unlocked': true,
          });
        }
      }
