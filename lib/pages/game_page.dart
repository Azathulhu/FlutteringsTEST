...
class _GamePageState extends State<GamePage> with SingleTickerProviderStateMixin {
  ...
  void _updateSpawner(double dt) {
    ...
    if (enemyService.isWaveComplete(wavePool[currentWave]!, enemies)) {
      if (currentWave < maxWaves) {
        ...
        // final wave cleared — handle sub-level completion flow:
        levelComplete = true;
        paused = true;

        // 1) Mark complete and unlock next sub-level (or next level default)
        // Use .then to avoid making this sync/await inside the game loop
        levelService.completeSubLevel(
          widget.subLevel['id'],
          widget.level['id'],
        ).then((_) {
          // 2) Return true to the previous page so it can refresh & reflect new unlocks
          if (Navigator.of(context).canPop()) {
            Navigator.of(context).pop(true);
          } else {
            // Fallback: if can't pop, show complete dialog and go to LevelSelectionPage
            _showCompleteDialog();
          }
        }).catchError((err) {
          // If anything goes wrong, fallback to dialog (and log)
          debugPrint('Error completing sub-level: $err');
          _showCompleteDialog();
        });
      }
    }
  ...
  void _showCompleteDialog() async {
    // Complete the sub-level in Supabase and unlock the next one (fallback)
